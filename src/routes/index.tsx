...
function Dashboard() {
  ...
  // simple ring metrics
  const stepsToday = 7820;
  const stepsGoal = 10000;
  const calorieToday = 480;
  const calorieGoal = 700;
  const minutesToday = todayActivities.filter((a) => a.completed).reduce((s, a) => s + a.durationMin, 0);
  ...
  return (
    <div className="px-5 lg:px-10 py-6 lg:py-10 space-y-8 max-w-7xl mx-auto">
      ...
      >
        <RingCard color="#4ade80" icon={<Footprints className="h-4 w-4" />} title="Schritte heute" value={stepsToday.toLocaleString("de-DE")} sub={`${Math.round((stepsToday / stepsGoal) * 100)}% des Tagesziels`} ring={<ProgressRing value={stepsToday} max={stepsGoal} color="#4ade80" sublabel="Schritte" />} />
        <RingCard color="#60a5fa" icon={<Flame className="h-4 w-4" />} title="Kalorien" value={`${calorieToday} kcal`} sub={`Ziel ${calorieGoal} kcal`} ring={<ProgressRing value={calorieToday} max={calorieGoal} color="#60a5fa" sublabel="kcal" />} />
        <RingCard color="#a78bfa" icon={<Clock className="h-4 w-4" />} title="Trainingszeit" value={`${minutesToday} min`} sub={`Tagesziel ${minutesGoal} min`} ring={<ProgressRing value={minutesToday} max={minutesGoal} color="#a78bfa" sublabel="min" />} />
      </motion.div>

      {/* Quick stats */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <StatPill icon={<TargetIcon className="h-4 w-4" />} label="Einheiten / Woche" value={`${weekDoneActs.length}/${weekActs.length}`} accent="#4ade80" />
        <StatPill icon={<CalIcon className="h-4 w-4" />} label="Aktive Tage" value={`${activeDays}`} accent="#60a5fa" />
        <StatPill icon={<TrendingUp className="h-4 w-4" />} label="Wochenminuten" value={`${weekMinutes}`} accent="#a78bfa" />
        <StatPill icon={<CheckCircle2 className="h-4 w-4" />} label="Habits heute" value={`${habitsDoneToday}/${habits.length}`} accent="#f0abfc" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 rounded-3xl border border-border/60 bg-card/60 p-5 lg:p-6 shadow-soft">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">{format(new Date(), "EEEE, d. MMMM", { locale: de })}</p>
            </div>
            <Link to="/calendar" className="text-xs text-primary hover:underline inline-flex items-center gap-1">Kalender <ChevronRight className="h-3 w-3" /></Link>
          </div>
        ...
        <div className="rounded-3xl border border-border/60 bg-card/60 p-5 lg:p-6 shadow-soft">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/goals" className="text-xs text-primary hover:underline inline-flex items-center gap-1">Alle <ChevronRight className="h-3 w-3" /></Link>
          </div>
              ...
              return (
                <div key={g.id}>
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <span className="text-xs text-muted-foreground tabular-nums">{progress}/{g.target}</span>
                  </div>
      ...
      <div className="rounded-3xl border border-border/60 bg-card/60 p-5 lg:p-6 shadow-soft">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          {upcoming.map((a) => (
            <div key={a.id} className="rounded-2xl border border-border/50 bg-background/40 p-3 transition-all hover:border-primary/40 hover:bg-accent/20">
              <div className="grid h-9 w-9 place-items-center rounded-lg mb-2" style={{ backgroundColor: `${a.color}22`, color: a.color }}>
                <ActivityIcon iconId={a.icon} color={a.color} />
              </div>
              <div className="text-sm font-medium truncate">{a.title}</div>
              <div className="mt-0.5 text-[11px] text-muted-foreground">
                {isSameDay(parseISO(a.date), new Date()) ? "Heute" : format(parseISO(a.date), "EEE d.MMM", { locale: de })}
...
function RingCard({ color, icon, title, value, sub, ring }: { color: string; icon: React.ReactNode; title: string; value: string; sub: string; ring: React.ReactNode }) {
  return (
    <div className="rounded-3xl border border-border/60 bg-card/60 p-5 shadow-soft flex items-center gap-4">
      {ring}
      <div className="min-w-0">
        <div className="inline-flex items-center gap-1.5 text-xs uppercase tracking-wider" style={{ color }}>
          {icon}
          {title}
        </div>
